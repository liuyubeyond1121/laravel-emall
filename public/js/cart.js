$(function() {
	...
	$('.del_d').click(function() {
			...
			var delUrl=$('#myUse').attr('delUrl') ;
			// var token=$(this).next().attr('token') ;
			console.log(id);
			$('.modal').fadeIn();
			$('.no').click(function(){
				$('.modal').fadeOut();
	...
	$(".foot_del").click(function() {
		...
		console.log(str);
		if(str.length>0){
            var delsUrl=$('#myUse').attr('delsUrl') ;
            // var token=$('#myUse').attr('token') ;
            // str._token=token;
            console.log(str);
			$('.modal').fadeIn();
			$('.no').click(function(){
			...
			$('.yes').click(function(){
				$.get(delsUrl,{'ids':str},function (data) {
					if (data){
...
function adddel(){
	...
	$(".add").each(function() {
		$(this).click(function() {
			...
			var token=$(this).prev().attr('token') ;
			obj = $(this) ;
			$.ajax({
				type: "GET",
				url: url,
				data: {id:id,num:num,"_token":token},
...
$('#go-buy').click(function(){
	...
	var idstr = str.join() ;
	var buyUrl = $('#myUse').attr('buyUrl') ;
	totalPrice=$('.susumOne').html();
	console.log(totalPrice);
	console.log(str);
	console.log(idstr);
	if(str.length>0){
		var url = buyUrl+'?ids='+idstr ;
