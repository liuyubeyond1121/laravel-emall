...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
		<a href="javascript:;" onclick="member_add('添加用户','{{asset('admin/user/create')}}','','510')" class="btn btn-primary radius">
			<i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> <span class="r">共有数据：<strong id="count">{{count($result)}}</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		@foreach($result as $v)
			<tr class="text-c">
				...
				<td>{{$v->id}}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('{{$v->username}}','{{asset('admin/user/'.$v->id)}}','{{$v->id}}','360','400')">{{$v->username}}</u></td>
				<td>{{$v->getSex->sex}}</td>
				<td>{{$v->phone}}</td>
				<td>{{$v->email}}</td>
				...
				<td>{{$v->addtime}}</td>
				@if($v->status==1)
					<td class="td-status"><span class="label label-success radius" onClick="member_stop(this,'{{$v->id}}')">{{$v->getStatus->status}}</span></td>
				{{--@elseif($v->status==8)--}}
				@else
					<td class="td-status"><span class="label label-defaunt radius" Onclick='member_start(this,"{{$v->id}}")'>{{$v->getStatus->status}}</span></td>
				@endif
				<td class="td-manage">
					@if($v->status==1)
					...
					@endif
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','{{asset('admin/user/'.$v->id.'/edit')}}','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
					<a style="text-decoration:none" class="ml-5" onClick="change_password('修改密码','{{asset('admin/user/changePwd/'.$v->id)}}','10001','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont">&#xe63f;</i></a>
...
function member_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		$.ajax({
			type: 'get',
			url: '{{asset('admin/user/userStop')}}/'+id,
			dataType: 'json',
			success: function(data){
			    if(data == true){
                    ...
                    $(obj).parents("tr").find(".td-status").html('<span class="label label-defaunt radius" Onclick=\'member_start(this,"{{$v->id}}")\'>已停用</span>');
                    $(obj).remove();
...
function member_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
		$.ajax({
			type: 'get',
			url: '{{asset('admin/user/userStart')}}/'+id ,
			dataType: 'json',
			success: function(data){
			    if(data==true){
