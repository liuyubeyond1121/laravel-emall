...
<div>
	...
	<div class="page-container">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
			...
			<a class="btn btn-primary radius" onclick="product_add('添加产品','{{asset('admin/product/create')}}')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加产品</a></span>
			...
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				@foreach($result as $v)
					<tr class="text-c va-m">
						...
						<td>{{$v->id}}</td>
						<td><a onClick="product_show('{{$v->name}}','{{asset('admin/product/'.$v->id)}}','{{$v->id}}')" href="javascript:;"><img width="60" class="product-thumb" @if($v->defaultImage) src="{{asset('emall/n4/'.$v->defaultImage->imagePath)}}" @else src="{{asset('emall/notImage.png')}}" @endif></a></td>
						<td class="text-l"><a style="text-decoration:none" onClick="product_show('{{$v->name}}','{{asset('admin/product/'.$v->id)}}','{{$v->id}}')" href="javascript:;">
							{{$v->name}}</a></td>
						<td class="text-l">{{$v->abstract}}</td>
						<td><span class="price">{{$v->salePrice}}</span></td>
						@if($v->isSale == 1)
						...
						@endif
						<td class="td-manage"><a style="text-decoration:none" onClick="product_stop(this,'{{$v->id}}')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>
							<a style="text-decoration:none" class="ml-5" onClick="product_edit('产品编辑','{{asset('admin/product/'.$v->id.'/edit')}}','{{$v->id}}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
							<a style="text-decoration:none" class="ml-5" onClick="product_del(this,'{{$v->id}}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
					</tr>
				@endforeach
				<tr align="center" >
					<td colspan="9"><div class="pagination" style="clear: both" >{{$result->links()}}</div></td>
				</tr>
				</tbody>
			</table>
		</div>
